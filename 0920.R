...
sales = c(12, 5, 8) # 매출액 단위 "억"

# 표형태 데이터프레임
df1 = data.frame(부서 = dept, 매출=sales, 분기="1분기")
df1

# 막대그래프(1. 그래프의 데이터 초기화, 2. 그래프 모양, 3. 제목과 라벨 설정)
ggplot(data = df1, aes(x=부서, y=매출)) + 
  geom_bar(stat = "identity") + 
  labs(title="팀별 영업실적", x = "부서명", y = "매출(억 원")
...
# scales를 install 함으로써 다시 library 설정
library(ggplot2)
...
ggplot(data = df1, aes(x=부서, y=매출)) + 
  geom_bar(stat = "identity", fill=c("red", "orange", "green"), color="black") + 
  labs(title="팀별 영업실적", x = "부서명", y = "매출(억 원")
...
# gdRdriver, RColorBrewer 색상파레트를 제공하는 두 가지 패키지
...
# 부서별 분기별 막대그래프 작성(조건1> 3분기 실적으로 추가, 조건2> gdRdriver 패키지의 팔레트에 있는 색상을 막대에 설정,
...
# 분기별로 사용자 지정색 설정(scale_fill_manual(values = cm.colors(3)))
ggplot(df, aes(x=부서, y=매출, fill=분기)) + 
  geom_bar(stat = "identity") + 
  scale_fill_manual(values = cm.colors(3))+
  labs(title="부서별, 분기별 영업실적", x = "부서명", y = "영업매출(억원")

# 묶은 세로 막대그래프(position="dodge")
ggplot(df, aes(x=부서, y=매출, fill=분기)) + 
...
#오른쪽에 있는 export에 pdf로 보내는 것도 있음. 근데 pdf로 하면 데이터 손실이 발생할 수 있으므로 사진이 더 나음
