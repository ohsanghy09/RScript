...
for (x in 1:iteration){
  # 동전 던지기 (추출개수 1개이므로 복원 또는 비목원 추출의 결과가 같다.)
...
# 원주율 구하는 실험(시뮬레이션)
# 한 변의 길이가 1인 사각형 1/4의 원이 포함되어 있는 경우 
...
for (i in 1:iteration) {
  ...
  # 거리가 1이하면 원 내의 점으로 판단해서 원 내의 점의 개수를 저장하는 Ncircle에 값을 누적
  if(dist <= 1){
    N.circle = N.circle + 1
  }
